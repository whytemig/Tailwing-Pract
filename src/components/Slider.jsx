...
const Slider = () => {
  ...
  return (
      <div className='max-w-[1240px] mx-auto px-4 py-16 relative flex justify-center items-center'>
          <BsArrowLeftCircleFill className='absolute top-[50%] text-3xl text-white cursor-pointer left-8' onClick={prevSlide}/>
          <BsArrowRightCircleFill className='absolute top-[50%] text-3xl text-white cursor-pointer right-8' onClick={nextSlide}/>
          {sliderData.map(({ url }, index) => {
              return  <div className={index === slide ? 'opacity-100' : 'opacity-0'}>
                  {index === slide && (<img src={url} alt="/" className='w-full rounded-md'/>)}
              </div>
          })}
      </div>
  )
